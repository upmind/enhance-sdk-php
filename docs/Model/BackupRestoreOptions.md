...
**restore_only_files** | **string[]** | Used in conjunction with the restoreFiles parameter, will limit the restoration to only the files/directories specified. | [optional]
...
**restore_postgresql_databases** | **string[]** | The list of PostgreSQL database names that need to be restored. If this list is not specified all the databases found in the backup snapshot will be restored, otherwise if this list is specified as empty, no database will be restored. | [optional]
