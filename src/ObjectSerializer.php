...
namespace Upmind\EnhanceSdk;

use GuzzleHttp\Psr7\Utils;
use Upmind\EnhanceSdk\Model\ModelInterface;
...
class ObjectSerializer
{
        ...
        if (is_object($data)) {
            ...
            if ($data instanceof ModelInterface) {
                ...
                foreach ($data::openAPITypes() as $property => $openAPIType) {
                    ...
                    $value = $data->$getter();
                    // Enum values are intentionally not validated on serialization so that
                    // values originating from newer API versions round-trip without throwing.
                    // Customised template (templates/ObjectSerializer.mustache).
    ...
    public static function deserialize($data, $class, $httpHeaders = null)
    {
        ...
        if (method_exists($class, 'getAllowableEnumValues')) {
            // Tolerate enum values from newer API versions that this generated SDK does
            // not yet know about: return the raw value instead of throwing. Customised
            // template (templates/ObjectSerializer.mustache); see generator-config.yml.
            return $data;
        } else {
            $data = is_string($data) ? json_decode($data) : $data;
            ...
            if (!empty($discriminator) && isset($data->{$discriminator}) && is_string($data->{$discriminator})) {
                $subclass = '\Upmind\EnhanceSdk\Model\\' . $data->{$discriminator};
